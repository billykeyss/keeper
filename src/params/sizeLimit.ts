...
export const sizeLimitParams = z.object({
  ...
  protected_slot: z.object({ min_in: z.number().nonnegative(), max_in: z.number().nonnegative() }).strict().nullable().optional(),
  measurement: z.enum(["total_length", "fork_length", "girth"]),
  unit: z.enum(["inch", "cm"]),
  over_slot_retention: z.object({ max_daily: z.number().int().nonnegative(), min_in: z.number().nonnegative().optional() }).strict().optional(),
